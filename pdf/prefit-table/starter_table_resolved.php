...
try {
    ...
    for ($row++; $row <= $rowmax; $row++) {
        for ($col = 1; $col <= $colmax; $col++) {
            $num = "Col " . $col . "/Row " . $row;
	    // RP swith from percentage to fixed width (25% to 150 )
            $optlist = "colwidth=150  fittextline={font=" . $font . " fontsize=10}";
     ...
     */
    do {
        $p->begin_page_ext(0, 0, "width=a4.height height=a4.height");
        ...
        $optlist = "header=1 rowheightdefault=auto " .
				...
				"stroke={{line=other}} ".
				"horshrinklimit=100% errorpolicy=return"; //MRE

        /* Place the table instance */
        $result = $p->fit_table($tbl, $llx, $lly, $urx, $ury, $optlist);
        if ($result ==  "_error") {
	    // RP
	    if ($p->get_errnum() == 3424){
		// Table width 295.58 exceeds fitbox width 250 (required shrinking factor 84.5796% is smaller than 'horshrinklimit'
		// for demonstration place the table instance
		// determine the required width of the talbe
		$result = $p->fit_table($tbl, $llx, $lly, 10000, $ury, $optlist . " blind");
		$tablewidth = $p->info_table($tbl, "tablewidth");
		$result = $p->fit_table($tbl, $llx, $lly, $llx+$tablewidth, $ury, $optlist . " rewind=-1");
		// if an error occur, die again.
		if ($result ==  "_error") {
		    die("Couldn't place table: [" . $p->get_errnum() . "]" . $p->get_errmsg());
		}
